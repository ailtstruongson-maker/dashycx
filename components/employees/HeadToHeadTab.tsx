...
import { getHeadToHeadAnalysis as getAnalysis } from '../../services/aiService';
...
const HeadToHeadTab = forwardRef<HTMLDivElement, HeadToHeadTabProps>(({ onExport, isExporting, colorThemes, ...props }, ref) => {
    ...
    const handleAiAnalysis = async (tableConfig: HeadToHeadTableConfig, tableRows: any[]) => {
        ...
        setAnalysis(null);
        try {
            const result = await getAnalysis(tableRows, tableConfig.metricType, tableConfig.selectedSubgroups);
            setAnalysis(result);
        } catch (error) {
