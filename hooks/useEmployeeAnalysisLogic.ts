...
export const useEmployeeAnalysisLogic = (activeTab: string, setActiveTab: (id: string) => void, defaultTabs: any[]) => {
    ...
    // --- MODAL SAVE HANDLERS ---
    const handleSaveTab = (tabName: string, icon: string, tabId?: string) => {
        if (tabId) {
    ...
    const handleSaveTable = (tableName: string, defaultSortColumnId?: string) => {
    ...
    const handleSaveColumn = (columnConfig: ColumnConfig) => {
        const { tabId, tableId } = modalState.data || {};
